...
Rynek samochodów używanych w Polsce charakteryzuje się ogromną dynamiką i różnorodnością. Wybór odpowiedniego pojazdu to proces wieloetapowy, w którym cena końcowa jest wypadkową nie tylko stanu technicznego, ale także marki, prestiżu oraz trendów rynkowych. Niniejszy projekt ma na celu kompleksową analizę struktury polskiego rynku wtórnego na podstawie obszernego zbioru ogłoszeń motoryzacyjnych. W ramach pracy dokonano analizy popularności poszczególnych marek i modeli, zbadano rozkłady cenowe w różnych segmentach (od aut miejskich po luksusowe marki włoskie) oraz zweryfikowano statystycznie wpływ kluczowych parametrów technicznych na wartość pojazdu. Projekt łączy w sobie elementy statystyki opisowej, testowania hipotez nieparametrycznych oraz modelowania ekonometrycznego.

## Pytania badawcze

W trakcie analizy postawiono następujące pytania badawcze, które ukierunkowały proces przetwarzania danych:

-   **Struktura rynku:** Jakie marki i modele samochodów dominują w polskich ogłoszeniach i jaki jest udział poszczególnych typów nadwozia oraz rodzajów paliwa?

-   **Determinanty ceny:** Które parametry numeryczne (rok produkcji, przebieg, moc silnika, pojemność) wykazują najsilniejszą korelację z ceną pojazdu?

-   **Wpływ cech jakościowych:** Czy rodzaj skrzyni biegów, typ napędu oraz stan pojazdu (nowy/używany) w sposób istotny statystycznie różnicują cenę ofertową?

-   **Utrata wartości:** Jak drastycznie różnią się ceny aut nowych i używanych w przypadku najpopularniejszych modeli na rynku?

-   **Modelowanie:** W jakim stopniu podstawowe cechy pojazdu pozwalają na precyzyjne przewidzenie jego ceny rynkowej?
...
Na podstawie przeprowadzonej analizy sformułowano następujące wnioski końcowe:

-   **Dominacja segmentu Premium i marek niemieckich:** Polski rynek wtórny opiera się na markach niemieckich (VW, BMW, Audi). Co ciekawe, mimo że najpopularniejszym modelem jest Opel Astra, to marki luksusowe generują największy ruch w górnych przedziałach cenowych.

-   **Kluczowe czynniki cenotwórcze:** Najważniejszym predyktorem wartości samochodu jest jego wiek (rok produkcji), co potwierdziła bardzo silna korelacja Spearmana ($rho = 0.83$). Drugim kluczowym czynnikiem jest przebieg, natomiast pojemność silnika straciła na znaczeniu ze względu na trend downsizingu.

-   **Wpływ skrzyni biegów i napędu:** Rodzaj skrzyni biegów okazał się jednym z najsilniejszych czynników różnicujących cenę (siła efektu 0.54). Samochody z automatyczną skrzynią biegów są wyceniane znacznie wyżej, co wynika z ich dominacji w droższych segmentach i rosnącej popularności wśród kierowców.

-   **Utrata wartości:** Analiza wykazała ogromny spadek wartości aut tuż po wyjeździe z salonu, szczególnie w segmencie premium (BMW Seria 3, Audi A4), gdzie różnica między medianą cen aut nowych i używanych przekracza 80%.

-   **Preferencje estetyczne:** Polacy preferują "bezpieczne" kolory – blisko 70% rynku to samochody czarne, szare, srebrne i białe, co ułatwia ich późniejszą odsprzedaż.

-   **Skuteczność modelu:** Zbudowany model regresji wielorakiej wyjaśnia aż 73% zmienności cen ($R^2 = 0.73$). Potwierdza to, że cena samochodu w Polsce nie jest przypadkowa i w dużej mierze zależy od obiektywnych cech technicznych, takich jak rok produkcji, moc silnika oraz rodzaj skrzyni biegów.
